...
contract RewardsDistributor is IRewardsDistributor {
    ...
    function _checkpoint_token() internal {
        ...
        uint since_last = block.timestamp - t; // time lapsed in seconds till last reward distribution time
        last_token_time = block.timestamp; // change last reward distribution time to current time
        uint this_week = (t / WEEK) * WEEK; // last time when rewards are distrubuted
        uint next_week = 0;
